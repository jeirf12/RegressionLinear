class Header {
  ...
  static createSelect(firstNode, {options, valueOptions, valueSelect, selectEvent, props}) {
    ...
    if(firstNode === null) root.appendChild(select);
    else root.insertBefore(select, firstNode);
    selectEvent(props);
  }

  static title(firstNode, {titleText, popup}) {
    let root = document.getElementById("content");
    let title = document.createElement("h4");
    title.textContent = titleText;
    if(firstNode === null) {
      root.appendChild(popup.init());
      root.appendChild(title);
    } else {
      root.insertBefore(popup.init(), firstNode);
      root.insertBefore(title, firstNode);
    }
  }
}
